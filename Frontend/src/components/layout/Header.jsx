...
const Header = () => {
  ...
  const { toggleCartDrawer, isLocationModalOpen, openLocationModal, closeLocationModal } = useUIStore();
  ...
  return (
    <>
          ...
          <div className="flex items-center justify-between h-16 gap-4">
            ...
            <div className="flex items-center gap-4 min-w-fit">
              ...
              <button
                ...
                className="hidden lg:flex flex-col items-start -space-y-0.5 group"
              >
                <span className="text-[10px] font-bold text-gray-900 uppercase tracking-wider">
                  Delivery in 10 mins
                </span>
                <div className="flex items-center gap-1 text-gray-700 group-hover:text-primary transition-colors">
                  <MapPin className="w-3.5 h-3.5 text-primary flex-shrink-0" />
                  <span className={`text-sm font-bold truncate max-w-[140px] ${!location ? 'text-primary' : 'text-gray-800'}`}>
      ...
      {/* Location Modal — rendered outside header so it overlays everything */}
      <LocationModal
